...
namespace MvcDynamicForms.NetCore.Fields.Abstract
{
    ...
    public abstract class ListField : InputField
    {
        ...
        public void AddChoices(string choices, string delimiter)
        {
            ...
            var choiceList = choices.Split(delimiter.ToCharArray(), StringSplitOptions.RemoveEmptyEntries)
                .Distinct()
                .ToList();

            foreach (var data in choiceList)
            {
                if (!_choices.Any(x => x.Value == data))
                {
                    this._choices.Add(new ListItem(data));
                }
            }
